...
class IrisProtocolClient {
  ...
  constructor(
    ...
    this.downloadsDirectory =
      options.downloadsDirectory || path.join(process.cwd(), "Downloads");
    ...
    this.in.subscribe((rawPacket) => {
      ...
      switch (packet.type) {
        case Constants.DOWNLOAD_FILE_ACCEPTED:
          ...
          const socket = dht.connect(fsPublicKey);
  ...
  setDownloadsDirectory(directoryPath) {
    this.downloadsDirectory = directoryPath;
  }
